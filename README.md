...
- **URL Templates** - Your configuration is saved in the URL for easy bookmarking and sharing
...
TimeCops automatically saves your stopwatch configuration in the URL, making it easy to create reusable templates:

1. Set up your stopwatches with custom names
2. The URL updates automatically (e.g., `timecops.html#timeclocks=Work&Break&Exercise`)
3. Bookmark the URL to save your template
4. Share the URL with others or use it across devices

**Note**: The URL stays clean on initial load. It only updates once you interact with the app (naming, adding, or deleting stopwatches).
...
#### Pomodoro Technique
```
timecops.html#timeclocks=Work&Short%20Break&Long%20Break
```
Set up timers for focused work sessions and breaks.

#### Workout Intervals
```
timecops.html#timeclocks=Warmup&Exercise%201&Exercise%202&Rest&Cooldown
```
Track different exercises and rest periods.

#### Cooking Multiple Dishes
```
timecops.html#timeclocks=Main%20Dish&Side%20Dish&Dessert
```
Time multiple cooking tasks simultaneously.

#### Team Meetings
```
timecops.html#timeclocks=Speaker%201&Speaker%202&Q%26A&Wrap%20Up
```
Track time allocation for different agenda items.
...
- **Privacy-First**: No data collection, no analytics, no cookies
